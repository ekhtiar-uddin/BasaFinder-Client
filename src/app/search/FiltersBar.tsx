...
const FiltersBarPage = () => {
  ...
  const handleFilterChange = async (
    ...
    const newFilters = { ...filters, location: searchInput, [key]: newValue };

    if (searchInput) {
      try {
        const res = await getCoordinates(searchInput);
        if (res.lat) {
          const lat = res.lat;
          const lng = res.lng;
  ...
  const handleLocationSearch = async () => {
    try {
      ...
      if (res.lat) {
        const lat = res?.lat;
        const lng = res?.lng;
        // console.log("here lat, lon", lat, lon);
        dispatch(
          setFilters({
            location: searchInput,
